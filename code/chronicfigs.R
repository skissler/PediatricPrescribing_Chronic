...
# 	ggplot(data=bind_rows(mutate(fulldat_cumrx,Comorbidities="No"), mutate(fulldat_cumrx_chronic,Comorbidities="Yes")), aes(x=AGE_DAYS_ROUNDED, y=NRX, col=Comorbidities, fill=Comorbidities, linetype=Comorbidities)) + 
# 		geom_ribbon(aes(ymin=lwr,ymax=upr),alpha=0.4) + 
# 		geom_line() + 
# 		scale_x_continuous(breaks=seq(from=0, to=1825, by=365)) + 
# 		theme_classic() + 
...
# 		scale_color_manual(values=c("Yes"="Red","No"="Black")) + 
# 		scale_fill_manual(values=c("Yes"="Red","No"="Black")) +
# 		scale_linetype_manual(values=c("Yes"="Red","No"="Black")) +
# 		theme(text=element_text(size=10))
